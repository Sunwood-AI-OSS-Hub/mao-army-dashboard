# 魔王軍団ダッシュボード - UI刷新 機能要件

> 世界征服を目指すAgent ZEROのエージェントチーム管理ダッシュボードのUI刷新に向けた機能要件定義
...
| 機能 | 説明 | コンポーネント/API |
|------|------|-------------------|
| チーム一覧表示 | 全チームの概要をカード形式で表示 | `page.tsx`, `TeamCard.tsx` |
| チームカード | メンバー数、タスク状況、進行状況バーを表示 | `TeamCard.tsx` |
...
| 魔王軍団テーマヘッダー | 炎のエフェクト、王冠アイコン | `DemonLordHeader.tsx` |
